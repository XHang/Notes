...
我现在有一个静态文件的存放目录，其实就是`webapps`目录啦，然后只需要登录

`http://127.0.0.1/xxxx`就可以拿到`webapps`目录的文件。
...
```
  server {
        ...
        location / {
            root   C:/Users/20160711/IdeaProjects/resume/webapps;
        }
        ....
        }
```

> 在windows里面，目录的分割线要求是`/`的
...
1. 初次运行没问题，能打印错误日志
   ...
   再次运行首先是发现日志没有打印了，其次是更改的配置也没有生效
   
   经过网上搜寻。。
   
   发现之前停止的nginx其实并没有真正停止，变成了守护进程了。。
   
   于是，端口继续被旧进程占用。新进程只是一个拥有内存空间的开盒子罢了
   
   所以这也就是为什么在网上搜寻时，总觉得生效的是旧配置
